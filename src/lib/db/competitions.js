...
export async function createCompetitionWithMatches(/** @type {any} */ competitionData, /** @type {any[]} */ matches) {
	// IDB's `put()` (see src/lib/db/idb.js) strips Svelte 5
	// reactive proxies via a JSON round-trip before writing,
	// so we can hand it raw $state values. We still tag the
	// match id + competitionId here because the engine builder
	// doesn't know the freshly-minted competition id yet.
