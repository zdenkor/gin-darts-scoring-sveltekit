...
import { playTurn as dartbotPlayTurn } from '../game/dartbot.js';
...
function renderSetup(router, { mode }) {
  ...
  (async () => {
    ...
    if (_ctx.user) {
      ...
      // Default to "Gin" / "Alex" on first run. The store.get
      // wrapper falls back to its second argument when the
      // localStorage key is missing — but if a previous code
      // path stored `null` explicitly (e.g. a test stub
      // calling store.set('lastPlayers', null)), JSON.parse
      // round-trips that as the JS value null, which would
      // crash `[...names]` two lines down. Guard against
      // that here.
      const saved = store.get('lastPlayers', null);
      names = Array.isArray(saved) && saved.length > 0 ? saved : ['Gin', 'Alex'];
    }
    // Parallel lookup: per-player bot config (index, level, originalName)
    // saved alongside lastPlayers so we can restore `originalName` on
    // resume. Without this, a bot that was previously toggled on (and
    // had its human name overwritten with "DartBot Level N" in
    // state.players) would lose the original name forever — the
    // toggle-OFF path in the bot toggle handler would fall back to
    // "Player N". lastPlayerBots is optional; if missing, we treat
    // every player as a human with no saved original name (the old
    // behaviour).
    const lastPlayerBots = store.get('lastPlayerBots', null) || [];
    const botsByIdx = new Map(lastPlayerBots.map(b => [b.index, b]));
    const state = {
      players: [...names], mode,
      ...
      // Shanghai
      n: 7,
      // Per-player bot config: one entry per player, parallel
      // to `state.players`. { isBot: false, level: 5 } for
      // human players, { isBot: true, level: N } for bots.
      // Only meaningful in x01 mode (the engine supports the
      // isBot field on any mode, but the setup UI only shows
      // the toggle for x01).
      //
      // When loading the last-used player list (e.g. a resume
      // from `store.get('lastPlayers', ...)`), check each
      // name for the "DartBot Level N" pattern. If we find
      // one, restore the bot config for that slot too — the
      // human will see the toggle already ON and the level
      // pre-selected, instead of an inert text input
      // showing a "DartBot Level 5" name (which our
      // substring filter would then block on next input,
      // and which would be passed to the engine as a
      // "human" named "DartBot …", silently breaking the
      // bot stats + visual cues).
      bots: (() => {
        const built = names.map((n, i) => {
          const saved = botsByIdx.get(i);
          if (saved) {
            // Resume path: the previous session saved per-player bot
            // info. Trust the saved level + originalName. If the saved
            // entry says isBot=true, the player slot is restored as a
            // bot (toggle on, name will be re-overwritten with the
            // canonical "DartBot Level N" on redraw). If isBot=false
            // we still want to keep originalName set to the human's
            // last accepted name — "Player N" by default — so the
            // toggle-OFF path can always restore a sensible value
            // even when the user never typed anything in the input.
            return {
              isBot: !!saved.isBot,
              level: Math.max(1, Math.min(15, Number(saved.level) || 5)),
              originalName: saved.originalName || `Player ${i + 1}`,
            };
          }
          // First-time / legacy path: derive bot state from the name
          // pattern (DartBot Level N → isBot=true). If the pattern
          // matches AND the name is exactly a bot name, we have no
          // human name to restore — fall back to "Player N" so the
          // toggle-OFF path always has something safe to write back.
          // If the pattern does NOT match, the player is a human and
          // the input field will show whatever they typed; we still
          // seed originalName with "Player N" so a later toggle
          // ON → toggle OFF round trip never leaves the slot empty.
          const m = /DartBot\s+Level\s+(\d+)/i.exec(n);
          if (m) {
            const lvl = Math.max(1, Math.min(15, Number(m[1]) || 5));
            return { isBot: true, level: lvl, originalName: `Player ${i + 1}` };
          }
          return { isBot: false, level: 5, originalName: `Player ${i + 1}` };
        });
        // De-duplicate bot levels. A previous (pre-all-bots-guard)
        // build of the app could persist a lastPlayerBots entry
        // with two bots at the same level (e.g. 5 and 5) — that
        // would mean the game has two slots whose canonical names
        // both read "DartBot Level 5", which collides in the
        // shared-history matchup column, the Stats screen, and
        // the engine's per-name accounting. Walk the bot slots
        // in order and bump any duplicate level up to the next
        // free value in 1..15. We pick the smallest free value
        // (not the next) so the resulting level assignment is
        // stable across reloads: if you reload the same setup
        // you get the same unique levels back, and the user can
        // see what was changed in the bot-level select. (We
        // intentionally do NOT touch human slots — their names
        // are user input, the level field is irrelevant for
        // them.)
        const usedLevels = new Set();
        for (const slot of built) {
          if (!slot.isBot) continue;
          let lvl = slot.level;
          // If this level is already used by an earlier bot
          // (or by ourselves — a level can never collide with
          // itself, so this loop always terminates after at
          // most 15 iterations), bump it.
          while (usedLevels.has(lvl)) {
            lvl = lvl >= 15 ? 1 : lvl + 1;
          }
          slot.level = lvl;
          usedLevels.add(lvl);
    ...
    // Helper: a "DartBot Level <N>" name to display in the
    // player list when the bot toggle is on. Used by
    // redrawPlayers to fill the select / input.
    const botNameFor = (lvl) => `DartBot Level ${lvl}`;
    ...
    function redrawPlayers() {
      ...
      state.players.forEach((name, i) => {
        ...
        // Self-heal: if state.players[idx] is a bot canonical
        // name (DartBot Level N) but state.bots[idx].isBot is
        // false, we're in an inconsistent state. The most
        // common path here is: a previous build persisted
        // lastPlayers=['DartBot Level 5', ...] while
        // lastPlayerBots had isBot:false (e.g. the user
        // toggled a bot OFF and the start handler didn't
        // sanitise the name), or the user loaded a legacy
        // lastPlayerBots that flipped isBot=false on a slot
        // whose persisted name is still a bot name. In all
        // such cases the input field below would otherwise
        // render "DartBot Level 5" with the input filter
        // blocking any keystroke (the filter rolls back to
        // the closure-captured `name`, which is the bot
        // name), leaving the user with an un-editable bot
        // name in a human slot. Replace the closure `name`
        // with the saved originalName (or "Player N" as a
        // last-resort default) and overwrite state.players
        // in lock-step so the rest of the code path sees a
        // consistent human name.
        const savedOriginal = state.bots[idx]?.originalName;
        const looksLikeBotName = /DartBot\s+Level\s+\d+/i.test(name || '');
        if (!state.bots[idx]?.isBot && looksLikeBotName) {
          const replacement = (savedOriginal || `Player ${idx + 1}`).trim() || `Player ${idx + 1}`;
          name = replacement;
          state.players[idx] = replacement;
          if (state.bots[idx]) state.bots[idx].originalName = replacement;
        }
        const row = el('div', { class: 'player-row' + (state.bots[idx]?.isBot ? ' bot-row' : '') });
        // Player number label ("P1", "P2", …).
        row.appendChild(el('span', { class: 'player-num' }, `P${idx + 1}`));
        // The name field: either a free-text input (human) or
        // a select listing "DartBot Level 1..15" (bot).
        let nameField;
        if (state.bots[idx]?.isBot) {
          nameField = el('select', { class: 'bot-level-select', onchange: e => {
            const lvl = Number(e.target.value);
            state.bots[idx].level = lvl;
            // Keep the player name in sync so the engine + UI
            // show the same identifier.
            state.players[idx] = botNameFor(lvl);
          } });
          for (let lvl = 1; lvl <= 15; lvl += 1) {
            const opt = el('option', { value: String(lvl) }, botNameFor(lvl));
            if (lvl === state.bots[idx].level) opt.selected = true;
            nameField.appendChild(opt);
          }
          // Ensure state.players[idx] matches the selected bot
          // level even if the user never touches the select.
          // Otherwise a slot loaded from lastPlayers with a
          // human name could keep that human name in
          // state.players while the UI shows a bot select,
          // which confuses lastPlayers sanitisation and any
          // code that reads state.players directly.
          state.players[idx] = botNameFor(state.bots[idx].level);
        } else {
          nameField = el('input', { type: 'text', value: name, placeholder: `Player ${idx + 1}`,
            oninput: e => {
              // Block the user from typing any name that
              // contains the string "DartBot" (case-
              // insensitive). The whole "DartBot …" namespace
              // is reserved for bot players so the engine /
              // UI can always tell whether a player is
              // human or bot by looking at the name.
              // Allowing "Gin DartBot" or "DartBot Jr." would
              // create collisions on substring matches
              // later; we forbid the substring outright.
              // We also forbid the bare "<level>" form (e.g.
              // "Level 1", "Level 5") that you would get by
              // stripping the "DartBot " prefix from the bot's
              // canonical name. Allowing it would let a human
              // player share a name with the bot they play
              // against (e.g. P1 = "Level 1", P2 = bot at
              // level 1) and the player cards in the game
              // screen would read identically, which makes
              // the match-up impossible to follow at a glance.
              const v = e.target.value;
              // Build a set of every name a bot could ever
              // take (DartBot Level 1 .. DartBot Level 15) and
              // compare the trimmed input against it
              // case-insensitively. This is a stricter guard
              // than the bare /DartBot/i substring test: a
              // user could otherwise type exactly "DartBot
              // Level 1" (which we want to forbid outright)
              // and the substring check would catch it, but
              // a deliberate future bot-name format change
              // (e.g. "Dart Bot L1", or a different bot kind
              // sharing the same /DartBot/i pattern) would
              // silently start leaking through. Listing the
              // canonical names is the source of truth here
              // — it follows whatever botNameFor() actually
              // produces, and is robust to whitespace /
              // case variations of the user input.
              const trimmed = (v || '').trim();
              const botNames = new Set();
              for (let lvl = 1; lvl <= 15; lvl += 1) {
                botNames.add(botNameFor(lvl).toLowerCase());
              }
              const isExactBotName = botNames.has(trimmed.toLowerCase());
              const isBareLevelName = /^Level\s+([1-9]|1[0-5])$/i.test(trimmed);
              if (/DartBot/i.test(v) || isExactBotName || isBareLevelName) {
                // Revert to the last accepted name and
                // flash a brief error tone. We keep the
                // input focus so the user can keep
                // typing.
                e.target.value = name;
                state.players[idx] = name;
                e.target.classList.add('input-rejected');
                setTimeout(() => e.target.classList.remove('input-rejected'), 250);
                return;
              }
              state.players[idx] = v;
            } });
        ...
        // Bot on/off toggle (slide switch, the same SVGs as
        // before). Hidden for cricket / shanghai (those modes
        // don't support a bot in this iteration). For x01
        // (mode === 'x01') it's always visible.
        if (mode === 'x01') {
          const isBot = !!state.bots[idx]?.isBot;
          const wrapId = `bot-toggle-${idx}`;
          const wrap = el('label', { for: wrapId, class: 'bot-toggle-wrap' });
          // The checkbox's `checked` must mirror state.bots[idx].isBot
          // at construction time. Without it, the input is always
          // unchecked (the default), so the first click on the
          // surrounding <label> TOGGLES the box from false to true —
          // even if state.bots[idx].isBot is already true and the
          // visual pill is showing the ON icon. The onchange handler
          // then sees `turned = true`, treats it as "user wants to
          // turn the bot on", and redraws the row in the same ON
          // state. The user sees the toggle "do nothing" the first
          // time. With `checked: isBot`, the box starts in the right
          // state and a single label click flips it to the opposite.
          const cb = el('input', { type: 'checkbox', id: wrapId, class: 'bot-toggle-input', checked: isBot,
            onchange: e => {
              const turned = !!e.target.checked;
              if (turned) {
                // Block the "all players are bots" configuration. The
                // game needs at least one human to drive the throw
                // entry / undo / dartpicker UI. If flipping this toggle
                // ON would make every player slot a bot, revert the
                // checkbox and toast a short explanation. The same
                // protection also stops a user from clicking a
                // disabled-looking toggle that the engine would have
                // to silently ignore later.
                //
                // Count "would-be bots" — i.e. the number of slots
                // that would still be a bot after this toggle lands.
                // Currently a bot + this toggle adds 1 (if the slot
                // was a human and is about to become a bot).
                const currentlyBots = state.bots.filter(b => b && b.isBot).length;
                const idxIsBot = !!(state.bots[idx] && state.bots[idx].isBot);
                const wouldBeBots = currentlyBots + (idxIsBot ? 0 : 1);
                if (wouldBeBots >= state.bots.length) {
                  e.target.checked = false;
                  pill.classList.toggle('on', false);
                  toast('At least one player must be human');
                  return;
                }
              }
              pill.classList.toggle('on', turned);
              state.bots[idx].isBot = turned;
              if (turned) {
                // Stash the human's name BEFORE we overwrite it
                // with the bot's canonical name. If they flip
                // the toggle back off later we can restore
                // "Gin" (or whatever they typed) instead of
                // falling back to the generic "Player N". The
                // substring filter on the name input still
                // blocks "DartBot" appearing in human names,
                // so this doesn't open a collision vector:
                // the saved name was typed by the user into
                // the human input, so it can't already be
                // "DartBot …".
                // Save the human's name so we can restore it on
                // toggle OFF. The uniqueness check happens in
                // the toggle OFF path (not here) because the
                // duplicate might not exist yet at this point —
                // e.g. the user toggles P1 to a bot (so "Gin"
                // is unique), then later types "Gin" into P2,
                // then tries to flip P1 back off. We only want
                // to bail out at the moment we'd be writing the
                // duplicate, not at the moment we stash the
                // would-be-restored value.
                //
                // Fallback to "Player N" if the input is empty
                // (or whitespace only) — we never want
                // originalName to be falsy, because the toggle-
                // OFF path uses it as a guaranteed-non-empty
                // value to write back into state.players[idx].
                const nameToStash = (state.players[idx] || '').trim() || `Player ${idx + 1}`;
                state.bots[idx].originalName = nameToStash;
                // Replace the free-text name with the bot's
                // canonical name, and force the level to the
                // current selection.
                state.players[idx] = botNameFor(state.bots[idx].level);
              } else {
                // Toggle OFF: demote this slot back to a human.
                // We deliberately do NOT restore the previous
                // human name here. The user wants a clean,
                // predictable reset: a bot becomes "Player N",
                // period. This prevents the confusing situation
                // where the previous session had a bot named
                // "DartBot Level 5"; the next session toggles
                // the bot OFF, and the input shows "DartBot
                // Level 5" (or some stashed name the user no
                // longer expects). Reset to "Player N" is
                // always safe, always unique, and always editable.
                state.players[idx] = `Player ${idx + 1}`;
                // Keep originalName in sync so a later toggle
                // ON → toggle OFF round trip has a guaranteed
                // non-empty value to stash (although we will
                // overwrite it again on the next ON anyway).
                state.bots[idx].originalName = `Player ${idx + 1}`;
              }
              redrawPlayers();
              // Persist the new bot config so a reload (without
              // going through Start) still preserves originalName.
              // Without this, if the user toggles ON a bot, reloads
              // the page, and clicks toggle OFF, the originalName
              // would be null (we never saved it) and the name
              // would fall back to "Player N" instead of "Gin".
              saveLastPlayerBots();
            } });
          const pill = el('span', { class: 'bot-toggle-pill' + (isBot ? ' on' : '') });
          pill.innerHTML = '<img src="assets/toggle/switch-off.svg" class="bot-toggle-icon off" alt="" draggable="false" />' +
                           '<img src="assets/toggle/switch-on.svg"  class="bot-toggle-icon on"  alt="" draggable="false" />';
          wrap.appendChild(cb);
          wrap.appendChild(pill);
          row.appendChild(wrap);
        }
        // Remove button (✕) — always shown, but the last
        // remaining row cannot be removed (engine requires at
        // least one player).
        const x = el('button', { class: 'x', title: 'Remove', onclick: () => {
    ...
    const addBtn = el('button', { class: 'btn ghost block', onclick: () => {
      if (state.players.length >= 8) { toast('Max 8 players'); return; }
      state.players.push(`Player ${state.players.length + 1}`);
      state.bots.push({ isBot: false, level: 5 });
      redrawPlayers();
    } }, '+ Add player');
    ...
    // Persist the current per-player bot config (parallel to
    // lastPlayers). Called immediately after a bot toggle so the
    // next time the user opens the setup screen — even if they
    // reload the page without pressing Start — the saved
    // originalName is available to restore on toggle OFF. Also
    // called from the Start button (below) so the final config is
    // always saved.
    //
    // The output array is keyed by index and parallel to
    // lastPlayers. Each entry is { index, isBot, level,
    // originalName }. originalName is the human name the user
    // had typed before they flipped the toggle ON — we stash it
    // in state.bots[idx].originalName on toggle ON and clear it
    // on toggle OFF, so what we save here is whatever's still in
    // memory at save time.
    function saveLastPlayerBots() {
      const playerBots = state.bots.map((b, i) => ({
        index: i,
        isBot: !!b.isBot,
        level: Math.max(1, Math.min(15, Number(b.level) || 5)),
        originalName: b.originalName || null,
      }));
      store.set('lastPlayerBots', playerBots);
    }

    const startBtn = el('button', { class: 'btn primary big block', onclick: () => {
      // Sanitise the names we are about to persist into
      // `lastPlayers`. lastPlayers is the long-term memory of
      // the player roster: it survives a reload AND is shown
      // as the default value of every name input the next
      // time the user opens the setup screen. If we let a
      // "DartBot Level N" name slip into it (because the slot
      // was loaded from a legacy lastPlayers with no
      // lastPlayerBots, or because the user managed to type
      // "DartBot" into a free-text input before the input
      // filter kicked in), the next setup screen would
      // pre-populate a HUMAN slot with a BOT name, the
      // substring filter on the input would then refuse
      // any keystroke, and toggle OFF would fail to restore
      // the human's original name (because the slot is
      // already showing a bot name). For every slot whose
      // bot toggle is on OR whose name matches the
      // "DartBot Level N" pattern, replace the persisted
      // name with the saved originalName (or "Player N" as
      // a last-resort default). The bot's canonical name is
      // re-derived later from `bots` and re-written into
      // the per-render `names` array just before the engine
      // gets it, so the engine still receives the right
      // thing.
      const sanitisedNames = state.players.map((n, i) => {
        const slot = state.bots[i] || {};
        const isBotSlot = !!slot.isBot;
        const looksLikeBotName = /DartBot\s+Level\s+\d+/i.test(n || '');
        if (isBotSlot || looksLikeBotName) {
          // For bot slots we never want a bot canonical name in
          // lastPlayers. The simplest, most predictable value is
          // the generic "Player N" placeholder. originalName is
          // only used to restore a human name on toggle OFF, but
          // now toggle OFF always resets to "Player N" anyway,
          // so there is no reason to persist a stashed human
          // name for a bot slot.
          return `Player ${i + 1}`;
        }
        return (n || '').trim();
      }).filter(Boolean);
      if (sanitisedNames.length < 1) { toast('Add at least one player'); return; }
      store.set('lastPlayers', sanitisedNames);
      // Also save per-player bot config (parallel to lastPlayers).
      // For each player slot emit { index, level, isBot, originalName }
      // so the next setup-screen open can restore the bot toggle AND
      // the originalName that the toggle handler stashed in the
      // current session. Without this, originalName would live only
      // in memory and be lost across reloads — the user would see
      // "DartBot Level 5" on resume, click toggle OFF, and watch
      // the name become "Player N" instead of their saved "Gin".
      saveLastPlayerBots();
      // Build the bots array from state.bots (parallel to
      // state.players). For each player whose toggle is on,
      // emit { index, level }. The renderGame handler reads
      // this and sets the corresponding player object fields
      // (isBot / botLevel / name) on the engine game.
      const bots = state.bots
        .map((b, i) => ({ index: i, level: b.level }))
        .filter((_, i) => state.bots[i] && state.bots[i].isBot);
      // If any bot is on, the name at that index may have been
      // mutated by redrawPlayers; sync it now so the engine
      // and the setup UI agree on the canonical bot name.
      for (const b of bots) {
        names[b.index] = botNameFor(b.level);
      }
      router.go('game', {
              mode, opts: modeOpts(state), names, bots,
...
function renderGame(router, params) {
  ...
  if (params.resume) {
    ...
    // Dartbot: if the setup screen tagged any player names as bots
    // (they start with "DartBot Level "), set the isBot / botLevel
    // flags on the corresponding player objects so the autoplay path
    // in afterThrow() picks them up. The name is preserved as-is
    // (already user-friendly: "DartBot Level 5").
    if (params.mode === 'x01' && Array.isArray(params.bots)) {
      for (const b of params.bots) {
        const idx = b.index;
        if (idx < 0 || idx >= game.players.length) continue;
        const p = game.players[idx];
        p.isBot = true;
        p.botLevel = Math.max(1, Math.min(15, Number(b.level) || 5));
        // Keep the human-readable bot name (e.g. "DartBot Level 5").
        p.name = `DartBot Level ${p.botLevel}`;
      }
    }
    game.rawDarts = [];
...
function openHistoryEdit(idx) {
  ...
  function render() {
    ...
    //
    // The `disabled` flag is set on the calculator's wrapper
    // when a DartBot thrower is active. The bot's autoplay
    // path submits the turn via setTimeout; we don't want
    // the human to be able to type / tap into the calc
    // during that window and double-submit. The calc stays
    // visible (so the user sees the layout / "no input
    // allowed" state) but every button / input is disabled
    // via the .calc-disabled CSS class (pointer-events: none
    // + reduced opacity).
    const isBotTurn = game.winner == null && !!game.players[game.current]?.isBot;
    if (game.type !== 'cricket' && game.type !== 'shanghai') {
      ...
      if (isBotTurn) {
        // Visually + functionally disable the calc. We don't
        // blank the buffer or replace the DOM; we just slap a
        // class on it and let CSS handle the rest. The
        // buttons + inputs keep their handlers, but
        // pointer-events: none makes them inert, and the
        // visual cue is dim opacity.
        calc.classList.add('calc-disabled');
      }
    ...
    if (game.type === 'cricket') {
      ...
      // Next ▶). Sits ABOVE the dartpad so the per-turn
      // commands are a single short row at the top of the
      // per-dart area, and the per-dart T/D/S picker is the
      // big block directly below them.
      ...
      // no triple bull in cricket). Buttons use the shared .btn
      // class for the visual style; T buttons get a ghost-variant
      // accent so the three columns are visually distinct.
  ...
  function commitTurnTotal(total) {
    ...
    // Snapshot the thrower index and bot flag BEFORE submit.
    // After submitTurnTotal01 advances `game.current` to the
    // next player, we can no longer tell which slot was
    // throwing this turn. For DartBot throwers we
    // auto-populate the checkoutAttempts stat instead of
    // asking the user (the bot knows what it threw).
    const throwerIndex = game.current;
    const throwerIsBot = !!game.players[throwerIndex]?.isBot;
    const preTurnScore = (game.players[throwerIndex]?.score != null)
      ? game.players[throwerIndex].score
      : null;
    ...
    // If the thrower is a DartBot, the bot knows exactly what
    // it threw, so we auto-populate the checkoutAttempts
    // stat instead of asking the user. The formula is the
    // same one the manual prompt uses (maxCheckoutAttemptsForX01),
    // guaranteeing the auto and manual paths produce the same
    // stat shape.
    let autoCheckoutAttempts = null;
    if (throwerIsBot && game.type === 'x01' && checkoutTarget != null) {
      const inOut = { in: game.opts?.in || 'single', out: game.opts?.out || 'single' };
      autoCheckoutAttempts = maxCheckoutAttemptsForX01(checkoutTarget, total, inOut, !!meta.isLegWin);
    }
    game.rawDarts.push({ total, ...meta, by: throwerName, ...(autoCheckoutAttempts != null ? { checkoutAttempts: autoCheckoutAttempts } : {}) });
    ...
    afterThrow();
    if (game.type === 'x01' && checkoutTarget != null && !throwerIsBot) {
      ...
      //
      // The `!throwerIsBot` guard skips the prompt for DartBot
      // throwers — the bot's checkoutAttempts was already
      // written into the entry above (autoCheckoutAttempts).
  ...
  function commitCricketDart(seg, mult = 1) {
    if (game.winner != null) return;
    if (!game.cricketDarts) game.cricketDarts = [];
    // Clamp multiplier to the per-segment max (3 for 15-20, 2
    // for bull=25, 0/1 for miss). The engine accepts
    // repeated-entries (one element per single mark) so a triple
    // bull is impossible — the upper bound is 2 marks on 25
    // (single + inner-bull = 2, the inner-bull counting as a
    // second hit, but the engine treats repeated 25 as additional
    // single-bull marks and caps at 2 anyway).
    const cap = (seg === 25) ? 2 : (seg === 0 ? 1 : 3);
    const m = Math.max(1, Math.min(mult, cap));
    game.cricketDarts.push({ segment: seg, multiplier: m });
    // Re-render immediately so the user sees the in-flight
    // marks (player.marks + this throw) light up on the
    // per-card cricket grid the instant they tap the button.
    // renderPlayerCard() projects game.cricketDarts onto the
    // mark tally, so this single render() updates every card
    // in lock-step with the new throw. endCricketTurn() runs
    // the same render() again after the engine applies the
    // turn, but with game.cricketDarts now [] — at that point
    // the grid reflects only the committed marks.
    render();
    // Auto-end at 3 darts (MAX_DARTS_PER_TURN). We count
    // cricketDarts.length as the number of darts thrown in this
    // turn — each element is exactly one throw, regardless of
    // whether it was a T, D, or S. (Previously the per-throw
    // mark count was pushed into the array, so a T20 took 3
    // slots and the turn auto-ended after the first dart of
    // any user input — making it impossible to throw T20 + D20
    // + 20 in the same turn.)
  ...
  function endCricketTurn() {
    ...
    // Expand each {segment, multiplier} throw into `multiplier`
    // repeated single-mark entries for the engine, which
    // processes segments as "one element = one mark". A T20
    // becomes [20, 20, 20], a D20 becomes [20, 20], a S20 is
    // [20], a 50 is [25, 25] (2 single-bull marks, which the
    // engine caps at the 2-mark bull need). The resulting flat
    // array is what submitTurnCricketMarks knows how to apply.
    const flatSegments = [];
    for (const d of segs) {
      const seg = d?.segment;
      const m = d?.multiplier || 1;
      if (seg == null) continue;
      for (let i = 0; i < m; i += 1) flatSegments.push(seg);
    }
    const result = submitTurnCricketMarks(game, flatSegments);
  ...
  function afterThrow() {
    ...
    // If the next thrower is a Dartbot, schedule its auto-throw
    // (with a short delay so it looks like a real player).
    maybeStartDartbotTurn();
  }

  // If the player whose turn it now is is a Dartbot, generate a
  // turn via the dartbot engine and submit it after a short delay.
  // The delay (1.2s by default) makes the bot feel like a real
  // player rather than snapping to the next throw instantly.
  // Recurses naturally: after submitting, afterThrow() runs and
  // calls maybeStartDartbotTurn() again, so back-to-back bot
  // turns (e.g. 1P vs Dartbot) keep going until a human's turn.
  // Also called from commitTurnTotal() at the end of a human
  // turn so the bot takes over as soon as the human is done.
  let dartbotTimeoutHandle = null;
  function maybeStartDartbotTurn() {
    if (dartbotTimeoutHandle != null) {
      clearTimeout(dartbotTimeoutHandle);
      dartbotTimeoutHandle = null;
    }
    if (game.winner != null) return;
    if (!game.players || !game.players.length) return;
    const next = game.players[game.current];
    if (!next || !next.isBot) return;
    // Only x01 is supported for the bot in this iteration.
    if (game.type !== 'x01') return;
    const level = Number(next.botLevel) || 5;
    const opts = game.opts || {};
    const remaining = next.score;
    dartbotTimeoutHandle = setTimeout(() => {
      dartbotTimeoutHandle = null;
      // Re-check the conditions — the user may have paused,
      // navigated away, or another bot may have already moved
      // during the delay.
      if (game.winner != null) return;
      if (!game.players[game.current] || !game.players[game.current].isBot) return;
      if (game.type !== 'x01') return;
      const playerNow = game.players[game.current];
      if (playerNow !== next) return; // turn changed underneath us
      const rem = playerNow.score;
      if (rem <= 0) return;
      const darts = dartbotPlayTurn(rem, { in: opts.in, out: opts.out }, level);
      const total = darts.reduce((s, d) => s + (d.segment * d.multiplier), 0);
      commitTurnTotal(total);
    }, 1200);
  }
  ...
  // If the first thrower (or whoever `game.current` points to
  // right now — e.g. a resumed game) is a DartBot, kick off
  // the autoplay chain immediately. Without this the bot
  // would only start after the human's first turn; the
  // expected behaviour is "P1, P2, P3, … in order, and the
  // bot doesn't wait for a human to act on its behalf".
  maybeStartDartbotTurn();
...
function renderPlayerCard(player, idx, game, onClick) {
  ...
  // Header row: player name + winner badge (when applicable).
  // The active-thrower cue is communicated through the green
  // border on the card itself, so we don't print "Throwing"
  // text here. DartBot players are distinguished by a purple
  // border around the .who / .score block (see CSS), not a
  // separate pill — this keeps the card header clean.
  ...
  if (game.type === 'cricket') {
    // The marks shown in this grid are the player's COMMITTED
    // marks (player.marks) plus the in-flight throws from this
    // turn (game.cricketDarts) that haven't been committed yet
    // by endCricketTurn. The user expects to see marks update
    // the moment they tap a T/D/S button on the dartpad — not
    // only after they hit Next ▶ and the engine applies the
    // turn. So we read the in-flight throws off game.cricketDarts
    // (which is reset to [] by endCricketTurn after the engine
    // commit) and project them onto the mark tally here.
    //
    // IMPORTANT: in-flight throws belong to ONE player — the
    // active thrower (game.current). Projecting them onto every
    // player's grid would make the inactive player's grid also
    // light up after the active thrower taps a button, which
    // is misleading. Only the current thrower's grid shows the
    // in-flight projection; everyone else sees their committed
    // marks only.
    const isCurrentThrower = game.current === idx;
    const inFlight = (isCurrentThrower && game.cricketDarts) || [];
    const projectedMarks = {};
    for (const n of [20, 19, 18, 17, 16, 15, 25]) {
      projectedMarks[n] = (player.marks && player.marks[n]) || 0;
    }
    for (const d of inFlight) {
      if (!d || d.segment == null) continue;
      const seg = d.segment;
      if (seg === 0) continue; // miss
      // Bull is segment 25 with multiplier 1 (single) or 2
      // (double); other segments use multiplier directly. The
      // cap matches the engine's behaviour (3 for 15..20, 2
      // for 25) so the on-screen tally never overshoots what
      // the engine will actually accept.
      const cap = seg === 25 ? 2 : 3;
      const m = Math.max(0, Math.min(d.multiplier || 1, cap));
      projectedMarks[seg] = (projectedMarks[seg] || 0) + m;
    }
    const grid = el('div', { class: 'cricket-grid' });
    [20, 19, 18, 17, 16, 15, 25].forEach(n => {
      const marks = projectedMarks[n] || 0;
