...
### Changed
- **Cricket game-screen layout** — the per-dart T/D/S picker
  and the action bar are now part of a single
  per-dart area below the player cards. The action bar
  (Undo, Redo, MoreCmds, Next ▶) sits ABOVE the dartpad
  so the per-turn commands are a single short row at the
  top of the per-dart area.
- **Cricket mark grid is back in the player cards.**
  After an experiment with a shared mark strip between
  the cards and the dartpad, the per-segment mark row
  (20, 19, 18, 17, 16, 15, BULL) is rendered inside
  every .player-card again — one row per card, one
  cell per segment, with marks / closed-state visuals.
- **Cricket mark grid is 3x larger** so the per-segment
  marks are easy to read at a glance on the game screen.
  Larger touch targets, larger cells, larger fonts.
- **In-flight marks light up the per-card grid the
  instant a T/D/S button is tapped.** The cricket-grid
  on each card projects `game.cricketDarts` (the
  in-flight throws for the current turn) onto the mark
  tally, so the user sees marks update immediately on
  tap, not just after they hit Next ▶ and the engine
  applies the turn. Only the current thrower's card
  shows the in-flight projection — the other players'
  grids continue to show their committed marks so the
  active thrower doesn't appear to push marks onto
  their opponent's tally mid-throw.
- **Cricket CSS is now fully responsive** — all px
  dimensions in the cricket-related rules (cell sizes,
  font sizes, padding, gap, border-radius, min-height)
  have been converted to vh, em, or clamp() expressions
  so the cricket grid scales smoothly with the rest of
  the UI on any viewport. 1px borders are kept as-is
  (standard for hairline borders).

### Fixed
- **Cricket dartpad now correctly counts darts, not
  marks, when checking the per-turn cap.** Previously,
  tapping T20 filled the turn buffer to 3 marks in a
  single throw, so the turn auto-ended before the user
  could press D20 or a single 20 in the same turn.
  `commitCricketDart` now pushes one `{segment,
  multiplier}` object per throw and the cap check uses
  `cricketDarts.length` (number of throws), not the
  sum of marks. T20 + D20 + 20 fits in one turn exactly
  as a real-world cricket turn should.

## [0.7.1] - 2026-07-03

### Changed
- **Dartbot: real-player simulator from L5+**. The engine
  in `js/game/dartbot.js` was retuned so that the bot plays
  like a real human, not a uniform sampler:
  - **L1-L4**: a child — swings at random low singles
    (1, 5, 10, 15, 20). No treble aim, no doubles, no
    board awareness.
  - **L5**: the first "real" dart player — single 20 is
    the safety throw, treble 20 starts appearing
    (≈ 0.7 weight out of the whole distribution), D20
    shows up as a setup dart.
  - **L8-L12**: a club / tournament player — T20 + T19
    dominate (T20 weight 2.5 → 4, T19 weight 1.5 → 2),
    single 20 covers the safe throw, D20 / D16 are
    common setup darts.
  - **L15**: pro — T20 is the bread-and-butter (weight
    4), T19 + T18 are the fallback, single 20 covers
    safety, near-zero weight on the bottom half of the
    board.
  - **Anchor points are now L1, L4, L5, L8, L12, L15**
    (was L1, L5, L10, L15) for finer resolution at the
    child → real-player transition (L4 → L5) and at the
    pro-grade end (L12 → L15).
- **`levelAccuracy` range tightened to 0.25 → 0.55**
  (was 0.30 → 0.95). Real-world reference: even a PDC
  pro only hits their intended target on ~50-60% of
  attempts; a casual pub player hovers around 5-15%.
  The previous 0.95 ceiling meant the bot "never
  missed" at L15, which inflated the round average to
  140+ and broke the spec'd ~100/round.
- **`aimAt` perturbation now level-driven** (was a
  fixed 50/30/15/5% distribution). At L15 a miss
  becomes a "same-number single" 60% of the time
  (treble → single — the right segment, wrong
  multiplier, which is the most common real-world
  miss). At L1 the miss scatters to neighbouring
  numbers 25% of the time and completely misses the
  board 15% of the time. Linear interpolation between
  L1 and L15.

### Result
- Round averages: L1 ≈ 25, L3 ≈ 32, L5 ≈ 46, L8 ≈ 72,
  L10 ≈ 82, L12 ≈ 91, L15 ≈ 99. Within ±10 of the
  spec'd L1=30, L5=50, L8=65, L10=75, L12=85, L15=100
  (L1-L3 run a bit low; L8+ are within tolerance).

## [0.7.0] - 2026-07-03

### Changed
- **DartBot: rebalanced as a real-darts-player simulator**.
  L1–L4 play like a child — random weak singles, no treble
  aim, no doubles. L5+ progressively learns to aim at the
  treble ring, with T20 as the bread-and-butter at L12+.
  Replaced the four 1/5/10/15 anchor distributions with six
  (1, 4, 5, 8, 12, 15) so the L1→L5 transition (child →
  real player) is visible and the L12→L15 high end has
  enough resolution for pro-grade play.
- **DartBot: rebalanced accuracy to real-world fidelity**.
  `levelAccuracy` now runs 0.25 (L1) → 0.55 (L15) instead
  of the previous 0.30 → 0.95. Real PDC pros hit their
  intended target on ~30-40% of attempts, not 95%. The
  previous 0.95 ceiling inflated the L12/L15 round average
  to ~140/round (way past the spec'd 100).
- **DartBot: level-dependent perturbation**. The miss
  distribution in `aimAt` is now level-driven: L1 misses
  scatter all over the board (30% same, 30% adjacent,
  25% neighbouring, 15% miss), L15 misses mostly land on
  the right number with the wrong multiplier (60% same,
  30% adjacent, 8% neighbouring, 2% miss). The cumulative
  thresholds interpolate linearly between the two anchors.

### Resulting averages
- L1: ~25/round (target 30)
- L5: ~46/round (target 50)
- L8: ~72/round (target 65)
- L10: ~82/round (target 75)
- L12: ~91/round (target 85)
- L15: ~99/round (target 100)

## [0.7.0] - 2026-07-03

### Added
- **DartBot — single-player AI opponent for x01**. New
  `js/game/dartbot.js` engine implementing 15 difficulty
  levels (Level 1 ≈ 30/round → Level 15 ≈ 100/round).
  The bot generates 3-dart turns using a hand-tuned
  per-level distribution (anchor points at L1, L5, L10, L15
  with piecewise-linear interpolation in between), and a
  checkout-aware planner that calls `checkoutSuggestions()`
  to find an optimal close-out (1- / 2- / 3-dart) when the
  remaining score is closable. Perturbation is level-driven:
  higher levels hit their aimed target more often, lower
  levels miss into adjacent segments / singles. Plan-aware
  bust handling: if a perturbation would over-throw, the
  affected dart becomes an explicit miss instead of
  busting the turn.
- **Per-player bot toggle in the setup screen** (x01 only).
  Each player row has a slide-switch pill on the right. OFF
  → free-text name input (human); ON → level-select with
  `DartBot Level 1` … `DartBot Level 15`. The toggle lives
  inside the existing players list (no separate card) so
  any number of players can be bots. Toggle pill uses
  free public-domain SVGs from freesvg.org
  (`assets/toggle/switch-{on,off}.svg`).
- **Auto-play chain**. `renderGame` schedules a 1.2s
  `setTimeout` for the bot's turn whenever `game.current`
  points at a `isBot` player. The chain re-arms after each
  bot throw (via `afterThrow → maybeStartDartbotTurn`),
  so back-to-back bot turns work without user input.
  Calculator is rendered but disabled (`pointer-events:
  none; opacity: 0.5`) while a bot thrower is active — the
  human cannot type / tap during the bot's 1.2s window.
- **Bot checkout attempts auto-populated**. The
  checkout-attempt modal is suppressed for bot throwers;
  `maxCheckoutAttemptsForX01` is called with the bot's
  actual throw and the result is written directly to the
  rawDarts entry, so checkout stats work for bot vs human
  games without prompting the human.
- **Bot visual cues in the scoreboard**. `isBot` players
  get a purple (`#8b5cf6`) border on their player card
  (flips to the standard green active-border when the bot
  is the current thrower).
- **Resume from `lastPlayers` restores bot config**. If the
  last-used player list contains a `DartBot Level N` name,
  the setup screen re-applies `state.bots[i].isBot = true`
  for that slot, so reopening the setup after a bot game
  shows the toggle already ON and the level pre-selected.
- **`DartBot` substring filter on human name input**. The
  human name input rejects any value containing the
  substring "DartBot" (case-insensitive) with a brief red
  flash, so a human cannot type a name that collides with
  the bot namespace.

### Tests
- `tests/dartbot.test.mjs` — 19 tests for the engine
  (level helpers, `aimAt`, `playTurn`, statistical
  averages).
- `tests/dartbot-integration.test.mjs` — 4 tests for the
  engine flow (isBot flag survives submission, full
  1vBot game reaches a winner).
